...
//生命周期
Object.assign(HUAWEI.prototype, {
  componentWillReceiveProps(nextProps) {
    let { haveListData } = this.state
    if (nextProps.tabActiveIndex === 1 && nextProps.subTabActiveIndex === 1) {
      if (haveListData) {
        console.log('华为再次进来时不会请求接口')
      } else {
        console.log('华为第一次会请求接口')
        this.setState({
          content: 'Loading...'
        })
        setTimeout(() => {
...
export default connect(mapStateToProps, mapDispatchToProps)(HUAWEI)